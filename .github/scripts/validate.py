...
SPONSOR_START = "<!-- sponsor:bisecthosting:start -->"
SPONSOR_END = "<!-- sponsor:bisecthosting:end -->"
...
def line(text: str, index: int) -> int:
    return text.count("\n", 0, index) + 1
...
def layout() -> None:
    ...
    dirs = [".github/ISSUE_TEMPLATE", ".github/workflows", "Releases", "Screenshots", "docs"]
...
def yaml_files() -> None:
    paths = sorted((ROOT / ".github/workflows").glob("*.y*ml")) + sorted((ROOT / ".github/ISSUE_TEMPLATE").glob("*.y*ml"))
    optional = [ROOT / "data/project.yml", ROOT / "data/mods.yml"]
    paths += [path for path in optional if path.exists()]
    errors: list[str] = []
    for path in paths:
        try:
            if yload(path) is None:
...
def metadata() -> None:
    ...
    if not project_path.exists() and not mods_path.exists():
        print("Metadata YAML skipped: data/project.yml and data/mods.yml are not part of this repo layout")
        return
    if not project_path.exists() or not mods_path.exists():
        fail("data/project.yml and data/mods.yml must either both exist or both be absent")

    project = yload(project_path)
    mods_doc = yload(mods_path)
    errors: list[str] = []
    expected = {
    ...
    for key, value in expected.items():
        if str(project.get(key)) != value:
            errors.append(f"data/project.yml {key}={project.get(key)!r}, expected {value!r}")

    sponsor = project.get("sponsor") or {}
    sponsor_expected = {
        "bisecthosting_url": SPONSOR_LINK,
        "required_banner_url": SPONSOR_BANNER,
        "sponsor_marker_start": SPONSOR_START,
        "sponsor_marker_end": SPONSOR_END,
    }
    for key, value in sponsor_expected.items():
    ...
    if not isinstance(mods, list) or not mods:
        errors.append("data/mods.yml must contain a non-empty mods list")
    if errors:
        fail("; ".join(errors))
...
def generated_docs() -> None:
    ...
    mods_path = ROOT / "data/mods.yml"
    if not project_path.exists() and not mods_path.exists():
        print("Generated docs freshness skipped: data source files are absent")
        return
    if not project_path.exists() or not mods_path.exists():
        fail("data/project.yml and data/mods.yml must either both exist or both be absent")
    ...
    path = ROOT / "latest-modlist.md"
    before = read(path)
    result = subprocess.run(
        [sys.executable, str(ROOT / ".github/scripts/generate_docs.py")],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    if before != read(path):
        fail("latest-modlist.md is stale; run python .github/scripts/generate_docs.py and commit the result")
...
def modlist() -> None:
    ...
    links = re.findall(r"(?<!!)\[[^\]\n]+\]\((https?://[^\s)]+)\)", text)
    errors: list[str] = []
    if "not a playable manifest" not in text.lower():
        errors.append("latest-modlist.md must say it is not a playable manifest")
    if len(links) < 40:
    ...
    for href in links:
        ...
        if (
            parsed.scheme != "https"
            or parsed.netloc != "www.curseforge.com"
            or len(parts) < 3
            or parts[0] != "minecraft"
            or parts[1] not in {"mc-mods", "texture-packs", "shaders", "modpacks"}
        ):
            errors.append(f"unexpected CurseForge link: {href}")
    errors += [f"duplicate link: {href}" for href, count in Counter(links).items() if count > 1]
    if errors:
        fail("; ".join(errors))
    print(f"Modlist OK: {len(links)} CurseForge links")
    ...
    for path in sorted(ROOT.glob("*.md")) + sorted((ROOT / "docs").glob("*.md")):
        ...
        for match in pattern.finditer(text):
            ...
            try:
                ...
                errors.append(f"{path.relative_to(ROOT)}:{line(text, match.start(1))} escapes repo: {href}")
                continue
            if not resolved.exists():
                errors.append(f"{path.relative_to(ROOT)}:{line(text, match.start(1))} broken local link {href}")
    if errors:
        fail("; ".join(errors))
...
def release_facts() -> None:
    ...
    facts = {
        ...
        "Forge": r"\bForge\b",
        "Java 17": r"\bJava\b[\s\S]{0,100}\b17\b|\b17\b[\s\S]{0,100}\bJava\b",
        "Project ID 1420406": r"\b1420406\b",
    }
    version_re = re.compile(r"\b(?:Still\s+Watching\s+V|Current\s+(?:documented\s+)?release[\s\S]{0,120}\bV|Latest\s+(?:documented\s+)?Version[\s\S]{0,120}\bV?)(\d+\.\d+\.\d+)\b", re.I)
    for path in FACT_FILES:
        text = read(path)
        for label, pattern in facts.items():
            if path.name != "latest-modlist.md" and not re.search(pattern, text, re.I):
...
def issue_templates() -> None:
    ...
    errors: list[str] = []
    if not config.is_file() or (yload(config) or {}).get("blank_issues_enabled") is not False:
        errors.append("config.yml must exist and set blank_issues_enabled: false")
    ...
    for path in forms:
        ...
        if not any(isinstance(item, dict) and (item.get("validations") or {}).get("required") is True for item in data.get("body") or []):
...
def sponsor_guard() -> None:
    ...
    desc = read(ROOT / "curseforge-description.html")
    errors: list[str] = []
    for label, value in {"start marker": SPONSOR_START, "end marker": SPONSOR_END, "link": SPONSOR_LINK, "banner": SPONSOR_BANNER}.items():
        if value not in readme:
            errors.append(f"README.md missing sponsor {label}")
    if SPONSOR_LINK not in desc or SPONSOR_BANNER not in desc:
        errors.append("curseforge-description.html missing sponsor link or banner")
    if errors:
        fail("; ".join(errors))
...
def all_checks() -> None:
    for check in [layout, yaml_files, html, metadata, generated_docs, local_links, readme_docs, curseforge_id_consistency, release_facts, issue_templates, modlist, sponsor_guard, release_zips]:
        check()
