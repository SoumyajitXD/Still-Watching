...
def metadata() -> None:
    errors = []
    project = yload(ROOT / "data/project.yml")
    mods_doc = yload(ROOT / "data/mods.yml")
    ...
    if not isinstance(mods, list) or not mods:
        ...
        for index, mod in enumerate(mods, 1):
            if not isinstance(mod, dict):
                errors.append(f"mod #{index} must be a mapping"); continue
    ...
    for path in sorted(ROOT.glob("*.md")) + sorted((ROOT / "docs").glob("*.md")):
        ...
        for match in pattern.finditer(text):
            ...
            try:
                ...
                errors.append(f"{path.relative_to(ROOT)}:{line(text, match.start(1))} escapes repo: {href}"); continue
...
def release_facts() -> None:
    ...
    for path in FACT_FILES:
        text = read(path)
        for label, pattern in {"Still Watching": r"\bStill\s+Watching\b", "Minecraft 1.20.1": r"\b1\.20\.1\b", "Forge": r"\bForge\b", "Java 17": r"\bJava\b[\s\S]{0,100}\b17\b|\b17\b[\s\S]{0,100}\bJava\b", "Project ID 1420406": r"\b1420406\b"}.items():
...
def issue_templates() -> None:
    ...
    for path in forms:
        ...
        if not isinstance(data, dict):
            errors.append(f"{path.name} must be a mapping"); continue
...
def main() -> None:
    checks = {"all": all_checks, "layout": layout, "yaml": yaml_files, "html": html, "metadata": metadata, "modlist": modlist, "markdown-links": local_links, "readme-docs": readme_docs, "curseforge-id": curseforge_id_consistency, "release-zips": release_zips, "release-facts": release_facts, "issue-templates": issue_templates, "sponsor": sponsor_guard}
